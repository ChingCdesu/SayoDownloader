...
let downloadItemData: IDownloadFile[] = getDownloadStore();
...
export const listenerDownload = async (
  ...
  item.on("done", (e, state) => {
    ...
    if (state === "completed") {
      ...
      if (store.get("openDownloaded")) {
        openFile(downloadItem.path);
      }
...
const listenerEvent = () => {
  ...
  // 新建下载
  ipcMainHandle("newDownloadFile", (event, data: INewDownloadFile) =>
    downloadFile(data)
  );

  // 重新下载
  ipcMainHandle("retryDownloadFile", (event, data: IDownloadFile) =>
    retryDownloadFile(data)
  );

  // 选择保存位置对话框
  ipcMainHandle("openFileDialog", (event, oldPath?: string) =>
  ...
  // 打开文件所在路径
  ipcMainHandle("openFileInFolder", (event, path: string) =>
    openFileInFolder(path)
  );

  // 暂停或恢复下载
  ipcMainHandle("pauseOrResume", (event, item: IDownloadFile) =>
    pauseOrResume(item)
  );
  ...
  ipcMainHandle("getDownloadItemCount", () => getDownloadItemCount());
